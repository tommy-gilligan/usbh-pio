...
pub fn calc_usb_crc5(mut data: u16) -> u8 {
    ...
    let lsb: u8 = (data >> 1) as u8 & 0x1f;
    let msb: u8 = (data >> 6) as u8 & 0x1f;

    let mut crc: u8 = if (data & 1) == 1 { 0x14 } else { 0x00 };
...
pub fn calc_usb_crc16(data: &[u8], len: usize) -> u16 {
    let mut crc: u16 = 0xffff;

    for idx in 0..len {
        crc = (crc >> 8) ^ CRC16_TBL[((crc ^ (data[idx] as u16)) & 0xff) as usize];
    }
...
mod test {
    #[test]
    fn test_calc_usb_crc5() {
        assert_eq!(super::calc_usb_crc5(23), 0x11);
        assert_eq!(super::calc_usb_crc5(39027), 0x0c);
    }
