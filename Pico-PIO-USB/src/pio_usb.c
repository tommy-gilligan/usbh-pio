...
void pio_usb_bus_init(pio_port_t *pp, const pio_usb_configuration_t *c, root_port_t *root) {
  ...
  root->pin_dp = c->pin_dp;
  root->pin_dm = c->pin_dp + 1;
  ...
  pp->offset_rx = pio_add_program(pp->pio_usb_rx, &usb_nrzi_decoder_program);
  pp->rx_reset_instr = pio_encode_jmp(pp->offset_rx);
  pp->rx_reset_instr2 = pio_encode_set(pio_x, 0);
  pp->offset_eop = pio_add_program(pp->pio_usb_rx, &usb_edge_detector_program);

  pio_sm_claim(pp->pio_usb_tx, pp->sm_tx);
  pio_sm_claim(pp->pio_usb_rx, pp->sm_rx);
  pio_sm_claim(pp->pio_usb_rx, pp->sm_eop);

  usb_tx_fs_program_init(pp->pio_usb_tx, pp->sm_tx, pp->offset_tx, root->pin_dp, root->pin_dm);
  usb_rx_fs_program_init(pp->pio_usb_rx, pp->sm_rx, pp->offset_rx, root->pin_dp, root->pin_dm);
  eop_detect_fs_program_init(pp->pio_usb_rx, c->sm_eop, pp->offset_eop, root->pin_dp, root->pin_dm, true);
  usb_tx_configure_pins(pp->pio_usb_tx, pp->sm_tx, root->pin_dp, root->pin_dm);
  pio_sm_set_jmp_pin(pp->pio_usb_rx, pp->sm_rx, root->pin_dp);
  pio_sm_set_jmp_pin(pp->pio_usb_rx, pp->sm_eop, root->pin_dm);
  pio_sm_set_in_pins(pp->pio_usb_rx, pp->sm_eop, root->pin_dp);

  dma_claim_mask(1<<c->tx_ch);
  dma_channel_config conf = dma_channel_get_default_config(c->tx_ch);
  channel_config_set_read_increment(&conf, true);
  channel_config_set_write_increment(&conf, false);
  channel_config_set_transfer_data_size(&conf, DMA_SIZE_8);
  channel_config_set_dreq(&conf, pio_get_dreq(pp->pio_usb_tx, c->sm_tx, true));
  dma_channel_set_config(c->tx_ch, &conf, false);
  dma_channel_set_write_addr(c->tx_ch, &(pp->pio_usb_tx)->txf[c->sm_tx], false);
}
